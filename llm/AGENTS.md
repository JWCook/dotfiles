...
- **Clear intent over clever code** - Be explicit and obvious. We prefer simple, clean, maintainable solutions over clever or complex ones, even if the latter are more concise or performant. Readability and maintainability are primary concerns.
...
- Use current python language features (python 3.13+) where relevant
...
- Dependencies: feel free to suggest additional dependencies that would make our job easier. Stop and ask for confirmation, and if applicable, provide a few different options of libraries to choose from.

# Tests
- Examine existing tests and follow the established style and structure.
- Be careful to test behavior, and not implementation details. Focus on the public API.
...
- Test function naming: Name tests in the format `test_{method_being_tested}`, for example `test_foo`. When there are multiple tests for the same method, optionally suffix test names with `__{description_of_test_condition}`, for example `test_foo__invalid_input`. Do not add superfluous suffixes like `_method` in `test_foo_method`.
